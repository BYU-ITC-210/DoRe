...
static class OAuth1p0 {
    ...
    public static bool ValidateOAuthSignature(string method, string url, IEnumerable<KeyValuePair<string, StringValues>> values, string secret) {
        ...
        if (!string.Equals(calcSig, foundSignature, StringComparison.OrdinalIgnoreCase))
            return false;
        if (foundTimestamp is null)
            return false;
        if (!long.TryParse(foundTimestamp, out var timestampLong))
            return false;
        var timestamp = DateTime.UnixEpoch.AddSeconds(timestampLong);
        if (DateTime.UtcNow - timestamp > s_timestampTolerance)
            return false;
