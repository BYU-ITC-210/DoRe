...
class SessionTokenManager {
    ...
    private string AddSignature(string token, DateTime expiration) {
        ...
        if (key is null)
            throw new InvalidOperationException("SignedToken: No keys set - cannot encode token.");
...
class SessionTokenKey {
    ...
    public SessionTokenKey(int id, byte[] key, DateTime created) {
        if (id < 0 || id > SessionTokenManager.KeyIdLimit)
            throw new ArgumentException("Out of range", "id");
        if (key.Length != SessionTokenManager.KeySize)
            throw new ArgumentException("Wrong Size", "key");
        Id = id;
        Key = key;
        Created = created;
    }
